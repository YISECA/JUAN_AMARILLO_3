      ...
      $usuario = Form::where('cedula', $request->input('cedula'))->first(); 
       ...
       if($this->inscritos()<=40){
        ...
        Mail::send('email', ['user' => $request->input('mail')], function ($m) use ($request) {
            ...
            $m->from('no-reply@idrd.gov.co', 'Registro Exitoso a esta expedición');



            $m->to($request->input('mail'), $request->input('primer_nombre'))->subject('Registro Exitoso a esta expedición!');
